...
# labmanager.cgi -
#
# Get/set EC-LabManager configuration info for UI
#
...
#        "/projects/@PLUGIN_NAME@/scm_driver/ECLabManager")) {
...
sub getCfgList {
    ...
    my $gcfg = new ElectricCommander::PropDB($::ec,"/projects/@PLUGIN_NAME@/labmanager_cfgs");
    ...
    foreach my $cfg (keys  %cfgs) {
        my $server = $gcfg->getCol("$cfg/labmanager_server");
        my $port = $gcfg->getCol("$cfg/labmanager_port");

        $xml .= "  <cfg>\n";
        $xml .= "     <name>$cfg</name>\n";
        $xml .= "     <server>" . xmlQuote($server) . "</server>\n";
        $xml .= "     <port>" . xmlQuote($port) . "</port>\n";
        $xml .= "  </cfg>\n";
    }
    $xml .= "</cfgs>\n";
