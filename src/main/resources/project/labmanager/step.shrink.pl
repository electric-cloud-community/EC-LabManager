...
sub main {
    ...
    foreach my $node ($nodeset->get_nodelist) {
        ...
        $deplist->{$input_handle}{conf}        = $conf;
        $deplist->{$input_handle}{vm}          = $vm;
        $deplist->{$input_handle}{org}         = $org;
        $deplist->{$input_handle}{work}        = $work;
...
sub deleteInstances {
    ...
    foreach my $handle (keys %{$deplist}) {
        ...
        my $xPath = $::ec->runProcedure(
            "$proj",
            {
               ...
               actualParameter => [
                                                               ...
                                                               { actualParameterName => "connection_config", value => "$lm_config" },
                                                               { actualParameterName => "labmanager_org",    value => "$deplist->{$handle}{org}" },
                                                               { actualParameterName => "labmanager_work",   value => "$deplist->{$handle}{work}" },
                                                               { actualParameterName => "labmanager_config", value => "$deplist->{$handle}{conf}" },
                                                               { actualParameterName => "tag",               value => "$deplist->{$handle}{tag}" },
                                                               { actualParameterName => "results",           value => "$deplist->{$handle}{resultspath}" },
                                                               { actualParameterName => "destination_tag",   value => "cleanup" },
                                  ],
            }
        );
        if ($xPath) {
        ...
        if ($outcome eq 'error') {
            my $cleanup_error = $::pdb->getProp("$deplist->{$handle}{resultspath}/$deplist->{$handle}{tag}/cleanup_error");
            if ($cleanup_error =~ m/No configuration exists with this name:/) {

                $deplist->{$handle}{state}  = "stopped";
                $deplist->{$handle}{result} = "success";
                $deplist->{$handle}{mesg}   = "Configuration was manually stopped or failed";
            ...
            $deplist->{$handle}{state}  = "pending";
            $deplist->{$handle}{result} = "error";
            $deplist->{$handle}{mesg}   = "Configuration still running";
        }
        else {
